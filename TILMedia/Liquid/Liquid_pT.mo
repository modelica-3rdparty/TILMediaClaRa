...
model Liquid_pT "Incompressible liquid model with p and T as independent variables"
  extends TILMedia.BaseClasses.PartialLiquid_pT(          liquidPointer=TILMedia.Internals.TILMediaExternalObject(
        "Liquid",
...
protected
  final parameter Integer computeFlags=TILMedia.Internals.calcComputeFlags(
      ...
      false);
equation
  (d,cp,beta) =TILMedia.Internals.Liquid.AdditionalObjectFunctions.properties_Txi(T, xi, liquidPointer);
  h =TILMedia.Liquid.ObjectFunctions.specificEnthalpy_Txi(T, xi, liquidPointer);
  s =TILMedia.Liquid.ObjectFunctions.specificEntropy_pTxi(
    p,
    T,
    xi,
    liquidPointer);
  ...
  transp.lambda,
  transp.eta,
  transp.sigma) =TILMedia.Internals.Liquid.AdditionalObjectFunctions.transportPropertyRecord_Txi(T, xi, liquidPointer);
  else

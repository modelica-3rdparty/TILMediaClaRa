within TILMedia.Internals.VLEFluid.InternalLibrary;
model VLEFluid_dT
  "Compressible fluid model with d, T and xi as independent variables"
  extends TILMedia.BaseClasses.PartialVLEFluid_dT(vleFluidPointer=TILMedia.Internals.TILMediaExternalObject(
          "VLEFluid",
          ...
          vleFluidType.nc,
          0,
          getInstanceName()), M_i={TILMedia.Internals.VLEFluid.ObjectFunctions.molarMass_n(i - 1, vleFluidPointer)
        for i in 1:vleFluidType.nc});
...
equation
  ...
  (crit.d, crit.h, crit.p, crit.s, crit.T) =TILMedia.Internals.VLEFluid.AdditionalObjectFunctions.cricondentherm_xi(xi,
    vleFluidPointer);
  //calculate molar mass
  M = M_i[1];

  //Calculate Main Properties of state
  h =TILMedia.Internals.VLEFluid.AdditionalObjectFunctionsWithDerivatives.ObjectFunctions.specificEnthalpy_dTxi(
    d,
    T,
    xi,
    vleFluidPointer);
  p =TILMedia.Internals.VLEFluid.AdditionalObjectFunctionsWithDerivatives.ObjectFunctions.pressure_dTxi(
    d,
    T,
    xi,
    vleFluidPointer);
  s =TILMedia.Internals.VLEFluid.AdditionalObjectFunctionsWithDerivatives.ObjectFunctions.specificEntropy_dTxi(
    d,
    T,
    xi,
    vleFluidPointer);
  ...
  (q,cp,cv,beta,kappa,drhodp_hxi,drhodh_pxi,drhodxi_ph,w,gamma) =
    TILMedia.Internals.VLEFluid.AdditionalObjectFunctions.additionalProperties_dTxi(
    d,
    T,
    xi,
    vleFluidPointer);
  ...
  //Calculate VLE Properties
    ...
    (VLE.d_l,VLE.h_l,VLE.p_l,VLE.s_l,VLE.T_l,VLE.xi_l,VLE.d_v,VLE.h_v,VLE.p_v,
      VLE.s_v,VLE.T_v,VLE.xi_v) =TILMedia.Internals.VLEFluid.AdditionalObjectFunctions.VLEProperties_dTxi(
    -1,
    T,
    xi,
    vleFluidPointer);
 ...
 transp.lambda,
 transp.eta,
 transp.sigma) =TILMedia.Internals.VLEFluid.AdditionalObjectFunctions.transportPropertyRecord_dTxi(
      d,
      T,
      xi,
      vleFluidPointer);
  ...
  if computeVLEAdditionalProperties then
      ...
      TILMedia.Internals.VLEFluid.AdditionalObjectFunctions.VLEAdditionalProperties_dTxi(
      -1,
      T,
      xi,
      vleFluidPointer);
  ...
  if computeVLETransportProperties then
      ...
      TILMedia.Internals.VLEFluid.AdditionalObjectFunctions.VLETransportPropertyRecord_dTxi(
      -1,
      T,
      xi,
      vleFluidPointer);

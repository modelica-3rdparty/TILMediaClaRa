...
model VLEFluid_pT
  ...
  extends TILMedia.BaseClasses.PartialVLEFluid_pT(vleFluidPointer=TILMedia.Internals.TILMediaExternalObject(
          "VLEFluid",
...
equation
  ...
  (crit.d, crit.h, crit.p, crit.s, crit.T) =TILMedia.Internals.VLEFluid.AdditionalObjectFunctions.cricondenbar_xi(xi,
    vleFluidPointer);
  //calculate molar mass
  M = M_i[1];

  //Calculate Main Properties of state
  d =TILMedia.Internals.VLEFluid.AdditionalObjectFunctionsWithDerivatives.ObjectFunctions.density_pTxi(
    p,
    T,
    xi,
    vleFluidPointer);
  h =TILMedia.Internals.VLEFluid.AdditionalObjectFunctionsWithDerivatives.ObjectFunctions.specificEnthalpy_pTxi(
    p,
    T,
    xi,
    vleFluidPointer);
  s =TILMedia.Internals.VLEFluid.AdditionalObjectFunctionsWithDerivatives.ObjectFunctions.specificEntropy_pTxi(
    p,
    T,
    xi,
    vleFluidPointer);
  ...
  (q,cp,cv,beta,kappa,drhodp_hxi,drhodh_pxi,drhodxi_ph,w,gamma) =
    TILMedia.Internals.VLEFluid.AdditionalObjectFunctions.additionalProperties_phxi(
    p,
    h,
    xi,
    vleFluidPointer);
 ...
 transp.lambda,
 transp.eta,
 transp.sigma) =TILMedia.Internals.VLEFluid.AdditionalObjectFunctions.transportPropertyRecord_phxi(
      p,
      h,
      xi,
      vleFluidPointer);
  ...
  if computeVLEAdditionalProperties then
      ...
      TILMedia.Internals.VLEFluid.AdditionalObjectFunctions.VLEAdditionalProperties_phxi(
      p,
      -1,
      xi,
      vleFluidPointer);
  ...
  if computeVLETransportProperties then
      ...
      TILMedia.Internals.VLEFluid.AdditionalObjectFunctions.VLETransportPropertyRecord_phxi(
      p,
      -1,
      xi,
      vleFluidPointer);

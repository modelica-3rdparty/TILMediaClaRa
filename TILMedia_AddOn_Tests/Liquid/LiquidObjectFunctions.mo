...
model LiquidObjectFunctions
  ...
  parameter Real hStart_a = TILMedia.LiquidObjectFunctions.specificEnthalpy_Txi(295.15, zeros(0), liquidPointer=liquid.liquidPointer);
  parameter Real hStart_b = TILMedia.LiquidObjectFunctions.specificEnthalpy_Txi(295.15, zeros(0), liquid.liquidPointer);
  Real h_c= TILMedia.LiquidObjectFunctions.specificEnthalpy_Txi(295.15+time*2, zeros(0), liquidPointer=liquid.liquidPointer);
  Real h_d= TILMedia.LiquidObjectFunctions.specificEnthalpy_Txi(295.15+time*2, zeros(0), liquid.liquidPointer);
  ...
  function MyF
    ...
    output Real y;
  algorithm
    y:=offset+TILMedia.LiquidObjectFunctions.specificEnthalpy_Txi(295.15+u, zeros(0), eo);
  end MyF;
